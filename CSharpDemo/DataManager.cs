...
namespace TRTCCSharpDemo
{
    ...
    class DataManager : IDisposable
    {
        ...
        public void setMainForm(TRTCMainForm mainForm)
        {
            mMainForm = mainForm;
        }
        ...
        */
        ...
        {
            JArray ci = JsonConvert.DeserializeObject<JArray>(json_msg_array); //反序列化
            //JObject c11 = ci<JObject>[0].ToObject();

            foreach (var ss in ci)
            {
                string s11 = ss["message_elem_array"].ToString();
                JArray cii = JsonConvert.DeserializeObject<JArray>(s11); //反序列化
                foreach (var sss in cii)
                {
                    string type = sss["elem_type"].ToString();
                    if (type == "0")
                    {
                        string s = ss["message_client_time"].ToString();
                        string token = ss["message_conv_id"].ToString();
                        string sender = ss["message_sender"].ToString();

                        string strContent = sss["text_elem_content"].ToString();
                        string strContent1 = sender + ":" + strContent;
                        mMainForm.setRichEditText(strContent1);
                    }
                }
            }

            //var b = System.Text.Encoding.Default.GetBytes(json_msg_array);
            //if (json_msg_array.IndexOf("[") != -1)
            //{
            //    json_msg_array = json_msg_array.Replace("[", "{");
            //}
            ...
            //JSONObject jsonObject = JSON.parseObject(json_msg_array);

            //var jsonParams = JsonConvert.DeserializeObject<dynamic>(System.Text.Encoding.UTF8.GetString(b));
            //获取dynamic里边的数据

            //string text1 = System.Text.Encoding.UTF8.GetString(b);
            //Newtonsoft.Json.Linq.JObject jObject = (Newtonsoft.Json.Linq.JObject)Newtonsoft.Json.JsonConvert.DeserializeObject(json_msg_array);
            //JObject jObject = JObject.Parse(json_msg_array);
            //string destId = jObject["message_client_time"].ToString();
